  ...
  return (
    <div className={`${theme?"bg-[#F4F4F4]":"bg-[#1e1d1d] "} transition-colors duration-500 min-h-screen pt-40 px-24`}>
      ...
      <div className="flex gap-20 mt-8 justify-center">
        ...
        <button onClick={()=>{if(JSON.parse(localStorage.getItem('finalUser'))){router.push('createblog')}}} className={`cursor-pointer z-500 ${theme?"bg-gray-100/80 text-black border-none":"text-white bg-black border-white border"}  absolute top-[-130px] right-[10px] w-[238px] flex justify-center items-center gap-2  auth-btn`}  ><FaPlus color="#9590d8" className="cursor-pointer"/> Create DevPost Now</button>          <div className={`${theme?"bg-[#D7E4EB] text-black border-black":"bg-[#111111] text-white border-[#525252]"}  border-2  border-dashed py-20 px-8 w-full h-[710px] rounded-2xl`}>
