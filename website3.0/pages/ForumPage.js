...
const Tag = ({ name }) => (
  <div
    ...
    className="bg-gray-200 px-4 py-1 text-gray-700 cursor-pointer hover:bg-[#deecf5] hover:text-[#6089a4] transition-all duration-200"
  >
    {name}
  </div>
);
...
const TopIssue = ({ title, index }) => (
  <div className="flex items-center 2xl:justify-between max-md:gap-2 ">
    ...
    <span
      className="text-[12px] w-[80%] overflow-hidden text-ellipsis"
...
function ForumPage({ theme, finalUser, setIsPopup, setMsg }) {
  ...
  async function fetchData() {
    try {
      ...
      for (let i = 0; i < a.length; i++) {
        ar.push({
          image1: a[i].authorImage,
          name: a[i].authorName,
          answers: a[i].solutions.length,
        });
      }
      setMostHelpful([...ar]);
      setLoading(false);
  ...
  const authors = Array.from(
    new Set(originalIssues.map((issue) => issue.authorName))
  );
  const labels = Array.from(
    new Set(originalIssues.flatMap((issue) => issue.tags))
  );
  ...
  const handleAuthorSelect = (author) => {
    if (selectedAuthor === author) {
      ...
      setCurrentIssues(originalIssues);
    } else {
      setSelectedAuthor(author);
      filterIssuesByAuthor(author);
    }
    setShowAuthorDropdown(false);
  };
  ...
  const handleLabelSelect = (label) => {
    if (selectedLabel === label) {
      ...
      filterIssuesByLabel(label);
    }
    setShowLabelDropdown(false);
  };

  const filterIssuesByAuthor = (author) => {
    const filteredIssues = originalIssues.filter(
      (issue) => issue.authorName === author
    );
    setCurrentIssues(filteredIssues);
  };

  // Filter issues based on selected label
  const filterIssuesByLabel = (label) => {
    const filteredIssues = originalIssues.filter((issue) =>
      issue.tags.includes(label)
    );
    setCurrentIssues(filteredIssues); // Update issues based on selected label
  };
          ...
          <div className="w-[15%]">
            <div>
              ...
              <div
                className={`${
                  theme ? "" : "text-white"
                } flex flex-col max-xl:flex-row max-xl:w-[100vw]  gap-1 mt-4 flex-wrap`}
              >
                <Sidebar01Item
                  title="View All"
                  ...
                  <div
                    ...
                    className={` ${
                      selectedTags.includes(name)
                        ? "bg-gray-400"
                        : "bg-gray-200"
                    } px-4 py-1 text-gray-700 cursor-pointer hover:bg-[#deecf5] hover:text-[#6089a4] transition-all duration-200`}
                  >
                    {name}
                  </div>
                ))}
          ...
          <div className="w-[85%] max-xl:w-[100%]  max-lg:w-[100%] gap-10 pl-16 pr-16 max-xl:pl-0 max-xl:pr-0 max-lg:flex-col flex">
            <div className="w-[80%] max-lg:w-[100%]">
                  ...
                  <div className="flex justify-between items-center h-full flex-wrap max-md:justify-start">
                    <div className="max-sm:relative max-sm:top-1 flex  max-sm:px-2 gap-5 max-sm:gap-6 max-sm:w-[100%] items-center ">
                      <div
                        ...
                        className="flex items-center gap-2 max-sm:text-[14px] cursor-pointer"
                      >
                    ...
                    <div
                      ...
                      } max-sm:h-[41px] relative max-sm:top-1 max-sm:border-b-[1px] max-sm:border-t-[1px] max-sm:relative max-sm:w-[100%] max-sm:px-2 gap-12 items-center`}
                    >
                      <div
                      ...
                      {showAuthorDropdown && (
                        <div className="absolute top-[41px] bg-white text-black rounded-md w-[160px] z-10">
                          {authors.map((author, index) => (
                            <div
                              key={index}
                              className={`p-2 hover:bg-[#82b8db] cursor-pointer ${
                            ...
                            {sortLabels.map((data) => {
                              return (
                                <p
                                  ...
                                  className="p-2 hover:bg-gray-200  text-sm border-t-gray-500 border-t-[1px] w-[100%]"
                                >
                                  {data}
                                </p>
                              );
                ...
                <div className="p-4">
                  {loading
                    ...
                    : currentIssues.map((issue, index) =>
                        (!isClosed && !issue.isCLose) ||
                        issue.discussionUsers ? (
                          <div
                            key={index}
                            ...
                            <div className=" flex max-md:flex-col max-md:items-start  items-center justify-between">
                              ...
                              <div className="flex max-md:pl-[50px] max-sm:gap-[1rem] max-sm:items-center items-center gap-6 max-sm:mt-2 text-gray-500 flex-wrap">
                                ...
                                <div
                                  ...
                                  className=" flex items-center gap-2 max-sm:text-[11px]"
                                >
                                  <FontAwesomeIcon
                                    color={`${
                                      finalUser &&
                                      issue?.likes?.includes(finalUser._id)
                                        ? "blue"
                                        : ""
                                    }`}
                                    icon={faHeart}
              ...
              <div className="mt-10 flex justify-between mb-20 max-sm:flex-col max-md:gap-4 max-md:justify-center">
                <div
                  className={`max-md:text-center ${theme ? "" : "text-white"}`}
                >
                  Total: {issues.length}
                </div>
                <div
                  className={`flex gap-5 items-center max-sm:gap-2 max-sm:justify-center ${
                    theme ? "" : "text-white"
                  }`}
                >
                  <button
                ...
                <div
                  ...
                  className={`cursor-pointer max-md:text-center ${
                    theme ? "" : "text-white"
                  }`}
                >
                  Go {"->"}
            ...
            <div className="max-lg:w-[100%] w-[25%]">
              <div
                className="bg-[#6089a4] rounded-full text-white py-5 px-3 text-center text-base cursor-pointer"
              ...
              <div className="border-[1px] border-gray-200 mt-10 text-lg rounded-lg p-4">
                ...
                >
                  {mostHelpful
                    .slice(0, mostHelpful.length >= 4 ? 4 : mostHelpful.length)
                    .map((user, index) => (
