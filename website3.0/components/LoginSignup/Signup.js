...
const Signup = ({ onClose, onLoginClick , onBack }) => {
  ...
  return (
    <div className={`bg-[rgba(255, 255, 255, 1)] border-dashed border-black border-[2px]  ${theme? "bg-slate-100 border-black":"bg-[#0f0c0c]  border-white"}  p-5 border-rounded1 lg:w-[500px] md:w-[500px] h-[530px] max-sm:w-[96vw]  md:m-auto max-sm:h-auto  relative select pt-16`}>
      {popup && (
        <Popup
          msg={error}
...
<button className={`absolute top-[0.5rem] left-[1.5rem] bg-transparent border-none ${theme?"text-black":"text-white"} text-2xl cursor-pointer h-auto hover:text-[#666]`} onClick={onBack}>
      ...
      <input
        ...
        value={email}
        className={`md:w-[65%] ${theme?"border-gray-500 text-black":"border-white text-white"} p-[10px] mb-[10px]  border-b-2  bg-none background-none text-black md:ml-[70px] max-sm:w-[100%] rounded-none border-[#837b7b] input-place`} 

        onChange={(e) => setEmail(e.target.value)}
      />
      <br />
      
      <p className={`text-end text-[12px] relative max-sm:text-center md:right-[80px] ${theme?"text-black":"text-white"}`}>
