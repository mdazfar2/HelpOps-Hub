    ...
    <>
      {!isSent && (
        <div className={`bg-[rgba(255, 255, 255, 1)] border-dashed border-[2px] ${theme ? "bg-slate-100 border-black" : "bg-[#0f0c0c] whiteshadow border-white"} p-5 border-rounded1 lg:w-[500px] md:w-[500px] max-sm:h-auto  w-[96vw] relative select`}>
          ...
          <h1 className={`text-center mt-[5px] ${theme ? "text-black" : "text-white"} text-[22px] font-bold max-sm:mt-[20px]`}>
          ...
          {!allShow && (
            ...
            &#8592; {/* Left arrow Unicode character */}
              ...
              <input
                ...
                value={email1}
                className={`w-[65%] p-[10px] mb-[10px] border-b-2 bg-none background-none text-black ml-[70px] rounded-none border-[#837b7b] input-place mt-[20px] max-sm:w-[90%] max-sm:left-0 max-sm:ml-0 ${theme ? "border-gray-500" : "border-white text-white"}`}
          ...
          {allShow && (
            <>
              <button className={`google-btn mt-[50px] md:w-3/5 max-sm:text-[13px] max-sm:max-w-[240px] lg:max-w-[270px] max-sm:w-[75%] p-2 rounded-[18px] ${theme ? "bg-white google-btn1 border-none" : "bg-[black] google-btn2 border-solid border-white border text-white"} cursor-pointer flex justify-center items-center m-auto gap-[18px] font-semibold`} onClick={() => signIn("google")}>
                <img className="w-[30px] h-[30px] ml-[5px]" src="new/google.webp" alt="Google" />
                Sign in with Google
              </button>
              <button className={`github-btn md:w-3/5 max-sm:w-[75%] max-sm:text-[13px] max-sm:max-w-[240px] lg:max-w-[270px] p-2 mt-4 rounded-[18px] ${theme ? "bg-white google-btn1 border-none" : "bg-[black] google-btn2 border-solid border-white border text-white"} cursor-pointer flex justify-center items-center m-auto gap-[18px] font-semibold`} onClick={() => signIn("github")}>
              ...
              <div>
                ...
                <div style={{ position: "relative" }}>
                  ...
                  <input
                    ...
                    value={password}
                    className={`md:w-[65%] p-[10px] mb-[10px] border-b-2 bg-none background-none text-black md:ml-[70px] max-sm:w-[100%]  rounded-none ${theme ? "border-gray-500" : "border-white text-white"} border-[#837b7b] input-place`}
                    type={`${showPassword ? "text" : "password"}`}
                    placeholder="Password"
                  />
                  {showPassword ? (
                    <FaEye color={`${theme ? "black" : "white"}`} className={`absolute bottom-[24%] md:right-[22%] text-[1.5rem] max-sm:right-[20px] cursor-pointer`} onClick={toggle} />
                  ) : (
                    <FaEyeSlash color={`${theme ? "black" : "white"}`} className="absolute bottom-[24%] md:right-[22%] text-[1.5rem] max-sm:right-[20px] cursor-pointer" onClick={toggle} />
                  )}
                </div>
              </div>
